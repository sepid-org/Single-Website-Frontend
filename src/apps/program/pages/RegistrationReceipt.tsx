...
} from '../../website-display/redux/features/program/Registration';
...
import { useGetUserPurchasesQuery } from '../../website-display/redux/features/sales/Purchase';
...
const RegistrationReceipt: FC<RegistrationReceiptPropsType> = ({
  ...
  const { programSlug, receiptId } = useParams();
  ...
  const { data: purchases } = useGetUserPurchasesQuery(
    { userId: registrationReceipt?.user?.id, programSlug },
    { skip: !Boolean(registrationReceipt?.user?.id) }
  )
    ...
    <Layout appbarMode='GENERAL'>
      <Grid container spacing={2} alignItems='flex-start'>
        <Grid item xs={12} sm={4}>
          <Stack spacing={2}>
            <Stack component={Paper} spacing={2} sx={{ padding: 1 }}>
              {userInfo &&
                <Fragment>
                  <Box>
                    <Stack alignItems={"center"} direction='row' spacing={1} marginBottom={1}>
                      <Avatar
